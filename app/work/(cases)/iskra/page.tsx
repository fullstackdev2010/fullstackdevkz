import AppSeoLandingSections from "@/components/work/AppSeoLandingSections";
import CaseStudyTemplate from "@/components/work/CaseStudyTemplate";
import { appSeoContent } from "@/lib/appSeoContent";

export const metadata = {
  title: "Iskra Trade - Mobile Catalog & Orders",
  description:
    "Iskra Trade mobile sales app with a fast catalog, clean ordering flow, and lightweight backend.",
  alternates: { canonical: "/work/iskra" },
  keywords: [
    "mobile catalog app",
    "sales ordering app",
    "field sales Android app",
    "B2B order app",
    "product catalog mobile app",
  ],
};

export default function Page() {
  const seo = appSeoContent.iskra;

  return (
    <>
      <CaseStudyTemplate
        title="Iskra Trade - Mobile Catalog & Orders"
        tagline="Iskra Trade mobile sales app with a fast, clean backend."
        palette={["#7AA2FF", "#8DF2D6", "#FFB3EC"]}
        heroImage="/demos/iskra/01.jpg"
        privacyHref="/work/iskra/privacy"
        privacyLabel="Privacy & Data Handling"
        kpis={[
          { label: "Customer satisfaction", value: "100%" },
          { label: "LCP (mobile)", value: "<= 2.3 s" },
          { label: "Order throughput", value: "+28%", note: "post-launch" },
        ]}
        gallery={[
          { src: "/demos/iskra/02.jpg", platform: "android", alt: "Iskra Trade home screen" },
          { src: "/demos/iskra/03.jpg", platform: "android", alt: "Iskra Trade product catalog" },
          { src: "/demos/iskra/07.jpg", platform: "android", alt: "Iskra Trade product detail screen" },
        ]}
      />
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
        <AppSeoLandingSections {...seo} />
      </div>
    </>
  );
}
